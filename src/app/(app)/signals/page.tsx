...
import {
  TrendingUp, TrendingDown, Minus, LogOut, ChevronDown,
  Filter, Zap, AlertCircle, Info, Copy, RefreshCw, Brain, Loader2,
} from "lucide-react";
...
function SignalRow({ s, idx, onTrade }: { s: Signal; idx: number; onTrade: (sig: Signal) => void }) {
  const [expanded, setExpanded] = useState(false);
  const cfg = ACTION_CFG[s.action];
  const Icon = cfg.icon;
  const tradeable = s.action === "BUY" || s.action === "SELL";
          ...
          <div className="flex items-center gap-2">
            ...
            {tradeable && (
              <button
                ...
                className="flex items-center gap-1 px-2 py-0.5 rounded bg-primary/15 text-primary border border-primary/30 text-[10px] font-bold hover:bg-primary/25 transition-colors opacity-0 group-hover:opacity-100"
              >
                <Copy className="w-2.5 h-2.5" /> Trade
              </button>
            )}
...
export default function SignalsPage() {
  ...
  const [signalSource, setSignalSource] = useState<"mock" | "live">("mock");
  ...
  // Load signals from Supabase on mount
  const loadSignals = useCallback(async () => {
    try {
      const res = await fetch("/api/signals");
      const data = await res.json();
      if (data.signals?.length > 0) {
        // Map DB columns → Signal shape
        const mapped: Signal[] = data.signals.map((s: Record<string, unknown>) => ({
          id: s.id as string,
          ticker: s.ticker as string,
          exchange: (s.exchange as string) ?? "—",
          market: s.market as "US" | "UAE" | "INDIA",
          action: s.action as Signal["action"],
          entry: (s.entry_price as number) ?? 0,
          sl: (s.stop_loss as number) ?? null,
          t1: (s.target_1 as number) ?? null,
          t2: (s.target_2 as number) ?? null,
          rr: (s.rr_ratio as number) ?? null,
          confidence: (s.confidence as number) ?? 70,
          risk: (s.risk_score as number) ?? 30,
          currency: (s.currency as string) ?? "$",
          generatedAt: new Date(s.generated_at as string).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" }),
          rationale: s.rationale as string,
          newsItem: (s.news_item as string) ?? "",
        }));
  ...
  // Run Morning Brain
  const runMorningBrain = async () => {
    setRunningBrain(true);
    setBrainStatus("Fetching price data & computing technicals…");
    try {
      const res = await fetch("/api/morning-brain", { method: "POST" });
      const data = await res.json();
      if (data.success) {
        setBrainStatus(`✓ ${data.count} signals generated by ${data.model}`);
        await loadSignals();
      } else {
        setBrainStatus(`✗ ${data.error}`);
      }
    } catch (e) {
      setBrainStatus(`✗ ${String(e)}`);
    } finally {
      setRunningBrain(false);
      setTimeout(() => setBrainStatus(null), 5000);
    }
  };

  const displaySignals = signalSource === "live" && liveSignals.length > 0 ? liveSignals : SIGNALS;
  ...
  const counts = {
    BUY:  displaySignals.filter((s) => s.action === "BUY").length,
    SELL: displaySignals.filter((s) => s.action === "SELL").length,
    HOLD: displaySignals.filter((s) => s.action === "HOLD").length,
    EXIT: displaySignals.filter((s) => s.action === "EXIT").length,
  };
    ...
    <div className="p-4 space-y-4 h-full overflow-auto">
      {tradeSignal && (
        <TradeModal
          ...
          prefillSymbol={tradeSignal.ticker}
          prefillMarket={tradeSignal.market}
          prefillPrice={tradeSignal.entry}
          prefillSide={tradeSignal.action === "SELL" ? "SHORT" : "LONG"}
          prefillSL={tradeSignal.sl ?? undefined}
          prefillTP={tradeSignal.t1 ?? undefined}
        />
      )}
      ...
      >
        <div>
          <div className="flex items-center gap-2">
            ...
            {signalSource === "live"
              ? <span className="text-[10px] px-2 py-0.5 rounded-full bg-primary/15 text-primary border border-primary/30 font-bold">● AI LIVE</span>
              : <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground border border-border font-bold">DEMO DATA</span>}
          </div>
          <p className="text-xs text-muted-foreground mt-0.5">
            {signalSource === "live" ? `${liveSignals.length} AI-generated signals` : `${SIGNALS.length} demo signals — click Run Brain to generate real signals`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {brainStatus && (
            <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              className={`text-[10px] ${brainStatus.startsWith("✓") ? "text-primary" : brainStatus.startsWith("✗") ? "text-destructive" : "text-yellow-400"}`}>
              {brainStatus}
            </motion.span>
          )}
          <button onClick={runMorningBrain} disabled={runningBrain}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 text-primary border border-primary/30 rounded-lg text-xs font-bold hover:bg-primary/20 transition-colors disabled:opacity-50">
            {runningBrain ? <Loader2 className="w-3 h-3 animate-spin" /> : <Brain className="w-3 h-3" />}
            {runningBrain ? "Running…" : "Run Brain"}
          </button>
          <button onClick={loadSignals} className="p-1.5 rounded-lg hover:bg-muted/40 transition-colors">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              <AnimatePresence>
                {filtered.map((s, i) => (
                  <SignalRow key={s.id} s={s} idx={i} onTrade={setTradeSignal} />
                ))}
              </AnimatePresence>
              {filtered.length === 0 && (
                <tr>
